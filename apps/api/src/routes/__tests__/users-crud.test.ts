...
describe('POST /users (create)', () => {
  it('rejects when body is malformed', async () => {
  ...
  it('rejects when password fails policy', async () => {
    validatePasswordMock.mockReturnValue({ valid: false, message: 'PASSWORD_TOO_SHORT' })
    const res = await buildApp().request('/users', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ username: 'bob', password: 'weak' }),
    })
    expect(res.status).toBe(400)
  ...
  it('returns 409 when the username is already taken', async () => {
    ...
    const res = await buildApp().request('/users', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ username: 'bob', password: 'Aa1aaaaa' }),
    })
    expect(res.status).toBe(409)
    expect(((await res.json()) as any).error).toBe('USERNAME_EXISTS')
  })

  it('creates a new user and returns 201 with no password fields', async () => {
    validatePasswordMock.mockReturnValue({ valid: true })
    ...
    const res = await buildApp().request('/users', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ username: 'bob', displayName: 'Bob', password: 'Aa1aaaaa' }),
    })
