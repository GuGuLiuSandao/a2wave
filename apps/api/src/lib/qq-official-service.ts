...
const QQ_GUILD_MESSAGES_INTENT = 1 << 30
const QQ_GUILD_DIRECT_MESSAGES_INTENT = 1 << 12
...
export function buildQQOfficialIntents(config: NormalizedQQOfficialConfig): number {
  let intents = 0
  if (config.enableGroupAndC2c) intents |= QQ_GROUP_AND_C2C_INTENT
  if (config.enableGuildMessages) intents |= QQ_GUILD_MESSAGES_INTENT
  if (config.enableGuildDirectMessages) intents |= QQ_GUILD_DIRECT_MESSAGES_INTENT
  return intents
}
...
export function classifyQQGatewayClose(code: number): {
  ...
  return {
    clearSession: code === 4004 || code === 9001 || code === 9005,
...
/** Normalize the five message dispatches supported by QQ's official Gateway. */
export function normalizeQQOfficialMessage(
  ...
  let scene: QQScene
  let senderOpenId = ''
  const details: Pick<QQOfficialMessageSnapshot, 'groupOpenId' | 'channelId' | 'guildId'> = {}
  if (eventType === 'GROUP_AT_MESSAGE_CREATE' || eventType === 'GROUP_MESSAGE_CREATE') {
    ...
    scene = 'guild'
    senderOpenId = stringValue(author.id)
    details.channelId = stringValue(data.channel_id)
    details.guildId = stringValue(data.guild_id) || undefined
    if (!details.channelId) return null
  } else if (eventType === 'DIRECT_MESSAGE_CREATE') {
    scene = 'guild_dm'
    senderOpenId = stringValue(author.id)
    details.guildId = stringValue(data.guild_id)
    if (!details.guildId) return null
  } else {
    return null
  }
  if (!senderOpenId) return null

  const mentionedBot = eventType === 'GROUP_AT_MESSAGE_CREATE' || eventType === 'AT_MESSAGE_CREATE'
...
export function shouldTriggerQQOfficialMessage(
  ...
  if (message.scene === 'group') {
    return (
      config.enableGroupAndC2c &&
      ((config.groupTriggerOnAt && message.mentionedBot) || config.groupTriggerOnNewMessage)
    )
  }
  if (message.scene === 'c2c') return config.enableGroupAndC2c
  if (message.scene === 'guild') return config.enableGuildMessages
  return config.enableGuildDirectMessages
}
...
export function buildQQOfficialConversationId(
  ...
  if (message.scene === 'group') {
    return `${appId}:group:${message.groupOpenId}:${message.senderOpenId}`
  }
  if (message.scene === 'c2c') return `${appId}:c2c:${message.senderOpenId}`
  if (message.scene === 'guild') {
    return `${appId}:guild:${message.channelId}:${message.senderOpenId}`
  }
  return `${appId}:guild_dm:${message.guildId}:${message.senderOpenId}`
}
...
export class QQOfficialApiClient {
  ...
  private target(info: QQOfficialChannelInfo): string {
    ...
    if (info.scene === 'c2c') return `/v2/users/${encodeURIComponent(info.sender_open_id)}`
    if (info.scene === 'guild') return `/channels/${encodeURIComponent(info.channel_id)}`
    return `/dms/${encodeURIComponent(info.guild_id)}`
  }
...
export class QQOfficialConnectionManager {
    ...
    if (payload.op === 10) {
      ...
      const identify = shouldResume
        ...
        : {
            ...
            d: {
              token,
              intents: buildQQOfficialIntents(connection.config),
  ...
  private async handleDispatch(
    ...
    const intent = message.content || 'Please review the attached files.'
    const common = {
  ...
  async sendRunResultByContext(
    ...
    const replyMode =
      info.scene === 'group'
        ...
        : info.scene === 'c2c'
          ? connection.config.c2cReplyMode
          : info.scene === 'guild'
            ? connection.config.guildReplyMode
            : connection.config.guildDmReplyMode
    if (replyMode === 'none') return
    const reply = replyMode === 'reply'
    const canUpload =
      connection.config.sendArtifactsAsFile && (info.scene === 'group' || info.scene === 'c2c')
    ...
    if (!canUpload || (info.scene !== 'group' && info.scene !== 'c2c')) return
