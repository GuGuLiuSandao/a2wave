...
describe('QQ Official Gateway messages', () => {
  ...
  it('normalizes and triggers an at-mentioned QQ group message', () => {
    ...
    expect(message && buildQQOfficialConversationId(config.appId, message)).toBe(
      '102000000:group:group-1:member-1',
    )
  })

  it('does not trigger ordinary group messages unless enabled', () => {
    const message = normalizeQQOfficialMessage('GROUP_MESSAGE_CREATE', {
      id: 'msg-2',
      content: 'hello',
      group_openid: 'group-1',
      author: { member_openid: 'member-1' },
    })
    expect(message && shouldTriggerQQOfficialMessage(config, message)).toBe(false)
    expect(
      message &&
        shouldTriggerQQOfficialMessage({ ...config, groupTriggerOnNewMessage: true }, message),
    ).toBe(true)
  })

  it.each([
    ['C2C_MESSAGE_CREATE', { author: { user_openid: 'user-1' } }, 'c2c'],
    ['AT_MESSAGE_CREATE', { author: { id: 'user-1' }, channel_id: 'channel-1' }, 'guild'],
    ['DIRECT_MESSAGE_CREATE', { author: { id: 'user-1' }, guild_id: 'dm-1' }, 'guild_dm'],
  ] as const)('normalizes %s as %s', (eventType, extra, scene) => {
    expect(
      normalizeQQOfficialMessage(eventType, { id: 'msg', content: 'hello', ...extra }),
    ).toMatchObject({ scene, senderOpenId: 'user-1' })
  })
