...
可接收五类官方事件：QQ 群 @ 消息、QQ 群普通消息、QQ 单聊、QQ 频道 @ 消息、QQ 频道私信。群聊默认仅在 @ 机器人时触发；「群聊中的所有新消息均触发」默认关闭，避免高流量群无意消耗大量 Run。QQ 单聊和已启用的频道私信收到人工消息时直接触发。

回复可以引用原消息、发送新消息或关闭。QQ 对被动回复有较短时效；若 Agent 执行结束时原消息回复锚点已经失效，a2wave 会按 AstrBot 的行为降级为发送新消息。群聊和 QQ 单聊支持把 Agent 产物先上传到腾讯媒体接口再发送；QQ 频道暂以 a2wave 下载链接回传产物。用户发来的官方附件会按平台统一的文件类型、数量和大小限制下载后交给 Agent。
