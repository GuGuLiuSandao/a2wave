...
| `a2wave setup --dir/--port/--image/--base-url/--health-timeout/--no-start` | Overrides. `--base-url` must be a pure origin (LAN/reverse-proxy installs; drives `CORS_ORIGIN` + cookie security; health still probes localhost). **`--image` is REQUIRED** until a public registry ships (no unusable default; build locally with `docker compose build`); validated against the docker reference charset. The `.env` persists a unique `COMPOSE_PROJECT_NAME` and every compose invocation passes it explicitly via `-p` (shell env beats `.env` in Compose precedence, so `-p` is the only override-proof channel) — same-basename install dirs can never share Docker volumes. Health accepts only `status:"ok"` — HTTP 200 + `degraded` fails the install |
