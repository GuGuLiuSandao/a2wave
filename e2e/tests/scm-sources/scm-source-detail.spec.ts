...
test.describe('SCM Source managed storage', () => {
  test('creates a Git source without asking for a container path', async ({ page }) => {
    ...
    try {
      ...
      // antd Segmented hides the real radio behind the styled label, so assert it is the
      // selected option rather than that it is visible — the point is managed-by-default.
      await expect(dialog.getByRole('radio', { name: /托管存储|Managed storage/ })).toBeChecked()
